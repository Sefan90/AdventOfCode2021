...
def part1():
    ...
    for line in data:
        ...
        if x1 == x2:
            ...
            if x1 < x2:
                if y1 < y2:
                    lines.append(set(str(x1+i)+","+str(y1+i) for i in range(x2-x1+1)))
                else:
                    lines.append(set(str(x2-i)+","+str(y2+i) for i in range(x2-x1+1)))
            else:
                if y1 < y2:
                    lines.append(set(str(x1-i)+","+str(y1+i) for i in range(x1-x2+1)))
                else:
                    lines.append(set(str(x2+i)+","+str(y2+i) for i in range(x1-x2+1)))
    output = set()
